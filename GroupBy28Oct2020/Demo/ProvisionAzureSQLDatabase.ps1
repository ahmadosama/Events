...
# An error is returned and stored in notexists variable if resource group exists
Get-AzSqlServer -ServerName $SQLServer -ResourceGroupName $ResourceGroup -ErrorVariable notexists -ErrorAction SilentlyContinue
...
# An error is returned and stored in notexists variable if resource group exists
Get-AzSqlDatabase -DatabaseName $SQLDatabase -ServerName $SQLServer -ResourceGroupName $ResourceGroup -ErrorVariable notexits -ErrorAction SilentlyContinue
...
$FirewallRuleName = "Home_" + (Get-Date).toString("yyyyMMddHHMMss") 
...
New-AzSqlServerFirewallRule @NewFirewallRule;
